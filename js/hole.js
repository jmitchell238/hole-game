...
function makeHole(x, z, name, isPlayer) {
  ...
  const wall = new THREE.Mesh(PIT_GEO, PIT_MAT);
  ...
  const ring = new THREE.Mesh(
    ...
    new THREE.MeshBasicMaterial({ color: isPlayer ? 0x58d68d : 0xc0392b }));
  ...
  tag.style.color = isPlayer ? '#58d68d' : '#fff';
  document.getElementById('tags').appendChild(tag);
  return { wall, cap, ring, tag, x, z, r: 12, name, isPlayer, tx: x, tz: z, retarget: 0 };
}
