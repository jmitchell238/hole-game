...
function generate() {
  WORLD = Math.round(rand(560, 760));
  islands = [];
  const want = 12 + ((Math.random()*4)|0);       // 12–15 islands
  let guard = 500;
  while (islands.length < want && guard-- > 0) {
    const R = islands.length === 0 ? rand(160, 215)   // home island is big
            : Math.random() < 0.25 ? rand(30, 55)     // rocky islets
            : rand(70, 165);
  ...
  const spots = [];                              // bots spread across islands
  for (let i = 0; i < 7; i++) {
    const isl = islands[1 + (i % (islands.length - 1))];
    spots.push(randPointOn(isl, 0.5));
  }
  this.botSpawns = spots;
}

// Water with wave glints, then each island: shallows, beach, grassy interior.
function islandGroundTexture() {
  const S = 4096;
  return canvasTex(S, S, g => {
    const sc = S / (2*WORLD);
    const X = w => (w + WORLD) * sc;
    g.fillStyle = '#2e6d8e'; g.fillRect(0, 0, S, S);
    g.strokeStyle = 'rgba(255,255,255,.06)'; g.lineWidth = 2.5;
    for (let i = 0; i < 300; i++) {
    ...
    for (const isl of islands) {
      blob(isl, 1.22, '#5da4bc');                       // shallows
      blob(isl, 1.0,  '#e2cf9a');                       // beach
      if (isl.R > 60) blob(isl, 0.68, '#7cbf6b');       // grass
    }
  });
}
